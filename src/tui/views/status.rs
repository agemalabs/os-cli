...
pub fn render(frame: &mut Frame, area: Rect, data: &StatusData, user_name: &str) {
    ...
    render_tasks(frame, content_chunks[0], &data.tasks, &data.overdue);
    render_right(
        frame,
        content_chunks[1],
        &data.decisions,
...
fn render_tasks(frame: &mut Frame, area: Rect, tasks: &[StatusTask], overdue: &[StatusTask]) {
    let mut lines = vec![
    ...
    if !overdue.is_empty() {
        for t in overdue {
            lines.push(Line::from(vec![
                ...
                Span::styled(&t.title, theme::danger_style()),
                Span::styled("  overdue", theme::danger_style()),
            ]));
        }
    }
    ...
    if tasks.is_empty() && overdue.is_empty() {
        ...
        for t in tasks {
            ...
            lines.push(Line::from(vec![
                Span::styled(format!("  {} ", marker), theme::label_style()),
                Span::styled(&t.title, theme::label_style()),
                Span::styled(due_text, theme::muted_style()),
            ]));
        }
    }
    ...
    if decisions.is_empty() {
        ...
        for d in decisions {
            lines.push(Line::from(vec![
                ...
                Span::styled(&d.project, theme::muted_style()),
                Span::styled(format!("  {}", d.title), theme::label_style()),
            ]));
        }
    }
